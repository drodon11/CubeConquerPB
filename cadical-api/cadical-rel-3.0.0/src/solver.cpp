...
/*------------------------------------------------------------------------*/
  ...
  void Solver::backtrackX(int nLevels) {
    internal->backtrackX(nLevels);
  }
  bool Solver::propagateX( ) {
    bool ok = internal->propagate();
    return ok;
  }
  void Solver::search_assume_decision(int lit) {
    internal->search_assume_decision(lit);
  }
  
  int Solver::value (int lit) {
