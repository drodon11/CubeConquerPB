...
for j in "${arr_dec[@]}"; do
    ...
    obj="$(cut -d'*' -f2 <<<$j)"
    echo "running $binary $formula $options --proof-log=$logfile.proof"
    output=`timeout $time $binary $formula $options --proof-log=$logfile.proof 2>&1 | awk '/^o|SATISFIABLE|.*Assertion.*/ {print $2}'`
...
for j in "${arr_dec[@]}"; do
    ...
    obj="$(cut -d'*' -f2 <<<$j)"
    echo "running $binary $formula $options --bits-learned=0 --bits-overflow=0 --bits-reduced=0 --proof-log=$logfile.proof"
    output=`timeout $time $binary $formula $options --bits-learned=0 --bits-overflow=0 --bits-reduced=0 --proof-log=$logfile.proof 2>&1 | awk '/^o|Error:|SATISFIABLE|.*Assertion.*/ {print $2}'`
    if [ "$output" != "" ] && [ "$output" != "$obj" ]; then
        errors=`expr 1000 + $errors`
        echo "wrong output: $output vs $obj"
    fi
...
for idx in "${!arr_modes[@]}"; do
    ...
    for j in "${arr_opt[@]}"; do
        formula="$(cut -d'*' -f1 <<<$j)"
        type="$(cut -d'/' -f1 <<<$j)"
        logfile="$logfolder/${formula}_${mode}_$lazy"
        mkdir -p `dirname $logfile`
        echo -n "" > $logfile.proof
        echo -n "" > $logfile.formula
        formula="$SCRIPTPATH/instances/$formula"
        ...
        obj="$(cut -d'*' -f2 <<<$j)"
        echo "running $binary $formula $options --opt-mode=$mode --cg-encoding=$lazy --proof-log=$logfile.proof"
        output=`timeout $time $binary $formula $options --opt-mode=$mode --cg-encoding=$lazy --proof-log=$logfile.proof 2>&1 | awk '/^o|Error:|UNSATISFIABLE|.*Assertion.*/ {print $2}'`
        if [ "$output" != "" ] && [ "$output" != "$obj" ]; then
            errors=`expr 1000 + $errors`
            echo "wrong output: $output vs $obj"
        fi
